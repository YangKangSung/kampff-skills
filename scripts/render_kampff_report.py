...
def render(analysis: dict, bundle: dict | None = None) -> str:
  ...
  <section class="card" id="trigger">
    ...
    cp_seed = {
        ...
        "preset_short": (_cp.get("text_ko_short") or "").strip(),
        "tone": (_cp.get("tone") or "peer").strip() or "peer",
    }
    cp_seed_json = (
        json.dumps(cp_seed, ensure_ascii=False)
        .replace("</", "<\\/")  # do not break </script>
    )  # raw JSON — never html-escape (JSON.parse needs quotes)
    community_section = (
        ...
        '<div class="cp-actions" style="display:flex;flex-wrap:wrap;gap:8px;align-items:center"><label class="muted" style="font-size:12px">tone <select id="cpTone" style="margin-left:4px;padding:6px 8px;border-radius:6px"><option value="peer" selected>peer · 동료 톤</option><option value="short">short · 2–3줄</option><option value="board">board · text_ko</option></select></label>'
        '<button type="button" id="cpGen">게시 초안 만들기</button>'
        '<button type="button" id="cpCopy">Copy</button>'
        '<button type="button" id="cpClear">Clear</button>'
        "</div>"
        '<textarea class="cp-out" id="cpOut" placeholder="seed 있으면 자동 채움 · 없으면 거부"></textarea>'
