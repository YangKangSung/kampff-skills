...
| `platform` | yes | `x` \| `facebook` \| `instagram` \| `reddit` \| `linkedin` \| `community` \| `other` |
| `scope` | yes | `profile` \| `post` \| `subreddit` \| `board` \| `group` \| `thread` |
| `collect` | yes | `post` \| `comment` \| `reply` \| `repost` |
| `match_people` | yes | 이 URL에서 긁을 `people[].id` 목록 |
| `search` | no | 플랫폼별 검색·필터 (아래) |
...
| `community` | `author_display`, `author_id` | 사내 게시판 작성자 검색 |

## 수집기 파이프라인

```
targets.json
    → URL별 fetch (API / export / 합법적 크롤)
    → 작성자 매칭 (aliases)
    → texts[] 정규화
